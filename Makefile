...
#
# Generated on 2026-09-07T17:12:14Z by kres undefined.
...
generate: helm-plugin-install  ## Generate .proto definitions.
	@$(MAKE) local-$@ DEST=./
	@docker run --rm -v $(PWD):/src -w /src -e GITHUB_TOKEN golang:$(GO_VERSION) go run ./cmd/kres gen
...
.PHONY: sign-image-kres
sign-image-kres:  ## Signs the image for kres. Requires interactive Google authentication.
	@test -n "$$GITHUB_TOKEN" || { \
	...
	@TMP=$$(mktemp -d) && trap 'rm -rf "$$TMP"' EXIT && \
	  ...
	  docker run --rm -p 127.0.0.1:8585:8585 \
	    ...
	    $(REGISTRY)/$(USERNAME)/kres:$(IMAGE_TAG)
